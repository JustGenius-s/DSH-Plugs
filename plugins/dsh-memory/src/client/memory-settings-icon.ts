...
const ICON_MARKER = 'data-dsh-memory-icon'

/**
 * The settings shell picks section icons from a private id switch and exposes
 * no icon field on settings.section. Unknown ids (including `memory`) fall
 * back to the generic gear. Patch the nav button until the slot grows an icon
 * contribution — same approach as Codex.
 */
export function installMemorySettingsIcon(getLabel: () => string): () => void {
  ...
  const sync = (): void => {
    ...
    for (const button of body.querySelectorAll<HTMLButtonElement>('nav button')) {
      ...
      currentIcon.replaceWith(createMemoryIcon(currentIcon))
    }
  }
...
function createMemoryIcon(source: SVGElement): SVGElement {
  ...
  icon.replaceChildren(...createThinkGlyph())
  return icon
}

/**
 * Paths from the shipped IconThinkOutline16 glyph — same visual language as
 * Models / Plugins nav icons, without importing the primitives package into
 * this DOM patcher.
 */
function createThinkGlyph(): SVGPathElement[] {
