...
import type { DiscoveredModel, ModelsOperations } from './operations.ts'
...
/**
 * The request modalities a pi-ai model may accept (`llm-pi-ai` catalog:
 * `text` and `image`). A model that does not declare `image` is treated
 * text-only by the adapter, so this selector is the GUI for the profile's
 * `input` array the stock page omits.
 */
const MODALITIES = ['text', 'image'] as const

/**
 * The pi-ai thinking levels a profile may declare, in escalation order
 * (`llm-pi-ai` catalog: off/minimal/low/medium/high/xhigh/max). Each key
 * selects a level; `off`'s value is `null` ("supported, send nothing")
 * and every other level's value is its own spelling unless a gateway needs a
 * different wire value.
 */
const THINKING_LEVELS = ['off', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max'] as const

/** A row's `input` modality array, defaulting to text-only when unset. */
function modalitiesOf(model: ModelDraft): readonly string[] {
  const value = model['input']
  return Array.isArray(value) ? value.filter((x): x is string => typeof x === 'string') : ['text']
}

/** A row's `reasoningEfforts` map, or `false` for a non-reasoning model, or empty when unset. */
function effortsOf(model: ModelDraft): Readonly<Record<string, unknown>> {
...
export function ModelListEditor(props: ModelListEditorProps): ReactNode {
  ...
  /** Toggle one input modality in a row's `input` array. */
  const toggleModality = (index: number, modality: string): void => {
    // `text` is the floor every protocol carries; removing it would write an
    // empty list (schema: "inherit the default", which is text) or an
    // image-only model. Only `image` joins or leaves.
  ...
  /** Toggle one thinking level in a row's `reasoningEfforts` map. */
  const toggleEffort = (index: number, level: string): void => {
    const current = { ...effortsOf(models[index]) }
    if (level in current) delete current[level]
    else current[level] = level === 'off' ? null : level
    // A map that offers nothing beyond "off" describes a non-reasoning model;
  ...
  const fetchModels = async (): Promise<void> => {
    ...
    try {
      const response = await operations.discoverModels(probe.settingsNs, {
