...
- **Fonts** — Settings → Codex → Fonts selects interface/conversation and code/terminal fonts independently. The searchable DSH menu lists installed font families on your computer (with local font access), previews each font, and prioritizes common monospace families for code. You can also enter comma-separated family names and reset either field to the DSH default. Preferences persist with the profile and apply live; changing the terminal font remeasures its canvas and PTY columns without restarting the shell.
...
- **File mentions open the Sidebar** — clicking a file named in an assistant's closing prose always previews it in the right Sidebar. DSH's own routing depends on how the file happened to be recorded: a path the mutation tools wrote opens the Sidebar, but a path declared only through `present` launches the desktop's default application instead — so the same chip had two different destinations. This makes it one destination, the one the delivery card's own primary button already uses. Holding a modifier key (⌘/Ctrl/Shift/Alt) still reaches the host's action, so "open in default app" stays available.
