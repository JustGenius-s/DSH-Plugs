...
- **Files panel** — session-cwd file tree with status badges, read-only preview and git diff view. Right-click any row for: add to the conversation as an `@path` chip (a file inserts `@path`, a folder a folder chip with the canonical trailing-slash `@dir/` mention, byte-identical to a settled pick from the built-in `@` picker), reveal in the native file manager, copy path, and copy relative path. Syntax highlighting uses shiki's `light-plus` / `dark-plus` (VSCode's default token colors) and follows the app's light/dark switch; line-number gutters stay pinned during horizontal scroll.
