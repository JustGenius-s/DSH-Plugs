...
export interface SidePanelsService {
  ...
  /**
   * Merge a patch into one instance's nav state (persisted, so a restored tab
   * keeps it). Used by panels that store per-instance identity — e.g. the
   * side-chat tab records the side session it owns.
   */
