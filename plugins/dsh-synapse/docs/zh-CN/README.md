...
5. 使用“分支”从已完成的回答创建替代路径。新建的分支会留在地图上并实时显示流式回答，不会跳转到原生对话界面。
...
双击卡片标题即可进入重命名输入框，详情侧栏保留铅笔按钮；底部的“隐藏”与“详情、会话”并列，详情面板也有隐藏按钮。
隐藏以一轮问答为单位，只改变地图，不删除消息、不归档会话，也不改变模型上下文。

- 隐藏后保留“已隐藏 1 轮”的小标记，后续对话和分支连线不变，其他卡片不会自动移动。
- 点击小标记即可恢复。
- 隐藏状态保存在服务端，刷新、重启和会话重放后仍保留；恢复时保留原来的卡片位置。
- 正在发送、生成、等待处理，或被当前追问/分支草稿引用的卡片暂不能隐藏。
- 折叠影响后续子树；隐藏只影响这一张卡片。隐藏一个已折叠的节点时，后续节点会显示出来。

地图不再提供原来的“删除分支”操作。会话归档请使用会话管理入口；
以前通过删除操作屏蔽的分支不会因本次更新自动恢复。

## 卡片状态

每张卡片显示一个状态徽标，反映该会话当前的情况：

| 状态 | 含义 |
|---|---|
| 待处理 | 会话**已阻塞，需要你介入**——审批、确认计划或回答问题。此时智能体不会继续，需到对话中处理。 |
| 进行中 | 智能体正在工作，卡片实时流式显示回答。 |
| 已完成 | 本轮已有回答且会话空闲。 |
| 失败 | 本轮出错结束，没有产生回答。 |
| 等待中 | 还没有回答，且当前没有在跑（新卡片，或插件还没收到该会话的状态）。 |

“待处理”会根据阻塞类型显示更具体的提示（需要审批 / 需要确认计划 / 需要回答问题），并且整张卡片会高亮描边，便于在地图上快速找到需要操作的分支。

状态直接来自 DSH 自身的会话信号（`running`、`pendingInteraction`、`completed`），与 DSH 侧边栏的指示灯一致，不额外推断。
...
- Synapse 的 `workspaces.json` 只保存画布元数据、卡片摘要、布局和分支锚点。
- 每张卡片只保存问题与最后回答的**摘要**（问题上限 600 字符，回答上限 2400 字符，超出以 `…` 结尾）。
- 点击卡片“详情”时，插件从 DSH 会话重新读取该轮的完整内容（全部助手步骤、每条工具调用的参数与输出），并按需缓存。
- 工具调用的输出不写入 `workspaces.json`，只记录次数与 `callId`。
- 删除 `workspaces.json` 会丢失画布布局，但不会删除 DSH 会话。
- 分支（fork）新建后即使还没有第一条消息也会保留在画布上，运行期间实时显示流式回答。
- DSH 启动后，插件会在后台补齐那些从未被投影过的历史分支（冷会话），让它们重新显示出来。
...
- 分支能否接到父会话的某个具体轮次，取决于 DSH 是否持久记录了分叉位置（`seedLength` 或 `session/end-seed` 事件）。DSH 未记录时（多数为子代理/派生会话），分支内容仍会显示，但作为独立卡片呈现，不体现父子连线。
